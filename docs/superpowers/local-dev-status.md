...
- **Plan U: Structured triage clarifications.** When `ATLAS_FF_STRUCTURED_TRIAGE=true`, the architect's `architect.triage.needs_input` events render as an inline `<TriageClarificationForm>` (one fieldset per question, widget kind inferred from text — `"X or Y?"` → single-select, `"Should we ...?"` / `"Do you want ...?"` → yes-no, otherwise free text input). On submit the form serializes the answers into a hyphenated multi-line block (`"- {question} → {answer}"`) and re-runs the same `action()` / `refineAction()` ChatPanel uses for the textarea — no engine change, no architect prompt change. Flag-OFF preserves today's flat-bullet rendering byte-for-byte (assertion in `ChatPanel.test.tsx`). Future plan can pair the architect's triage step with a structured tool-use schema and replace heuristic inference with declared widget kinds; this slice ships the form contract so that upgrade is purely additive.
