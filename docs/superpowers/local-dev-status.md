...
Quick reference for "what does the app actually do when I click Send?" Last updated **2026-05-24 (evening)** — Plan U full slice 2 (architect-declared widget kinds), 4 of 10 legacy E2E specs rewritten + 6 with surgical un-skip rationale (ama-happy / multi-viewport / diego-happy / pr-flow now active; ama-bootstrap / diego-changes-requested / persona-toggle / drift-recovery / latency / priya-risk-accept documented as infra-blocked or product-gap), plan-f auto-reload flake fix via fresh-SSE handshake, plan-g rail-shell un-skipped after self-heal + cold-compile timeout bump. Active E2E: 17 specs passing, 10 documented skips, 0 failed. Atlas-web vitest: 790/3/0. Real-stack pipeline (architect → developer → sandbox-apply → preview) end-to-end green.
...
- **Plan SPU: Stunning pipeline upgrade (2026-05-12).** The ritual pipeline now produces website-quality output matching v0 / Lovable. Three independently flippable additions: (1) `ATLAS_FF_DESIGNER_CRITIQUE=true` switches DesignerRole from single-pass to three-pass — `draftProposal` (Haiku, cheap) → `critiqueDraft` (Haiku, 5-axis scoring) → `reviseDraft` (Sonnet, refined). Emits `designer.draft.completed` → `designer.critique.{started,completed}` → `designer.revise.{started,completed}` → `designer.proposal.{completed,emitted}`. (2) `ATLAS_FF_REFERENCE_IMAGES=true` plumbs user-uploaded reference imagery through `StartInput.referenceImages` → architect priorArtifact → designer prompt. Conditional spread keeps flag-OFF behavior byte-identical. (3) `ATLAS_FF_ASSET_GEN=true` registers `@atlas/role-asset-generator` in the factory; the engine dispatches it after canvas pause; the resulting `assetManifest` (hero + sections) folds into developer priorArtifact; render-user-turn.ts surfaces the URLs verbatim so the developer never invents image URLs. Three branches inside the role: gradient fallback (always works, zero-cost), `ATLAS_FF_HERO_UNSPLASH=true` (calls Unsplash with the project category, needs `UNSPLASH_ACCESS_KEY`), `ATLAS_FF_HERO_AI_IMAGE=true` (calls `api.openai.com/v1/images/generations` with `gpt-image-1`, needs a **real** OpenAI key — OpenRouter does NOT proxy image gen). sha256 content-addressable cache lands under `.next/cache/atlas-assets/`; the `/atlas-assets/:hash.jpg` rewrite serves them.
- **Plan UXO: Canvas UX overhaul (2026-05-12).** Six independently flippable UX changes lift the canvas to May-2026 SOTA. (1) `ATLAS_FF_PROMPT_MORPH=true` mounts `<PromptForm />` as the hero on `/` for signed-in users; the View Transitions API morphs the textarea into the canvas chat input on submit (Firefox-safe via `"startViewTransition" in document` guard). (2) `ATLAS_FF_MODE_TOOLBAR=true` renders a three-mode (Agent / Plan / Visual-Edits) radiogroup in the canvas header; mode persists per-project via localStorage; downstream consumers read via `useCanvasMode`. (3) `ATLAS_FF_CLICK_TO_EDIT=true` ships `atlas-edit-bridge.ts` inside the `atlas-next-ts` template — the sandbox posts a DOM tree to the parent over `postMessage`; `<IframeOverlay />` (mounted in `CanvasPreviewClient` when mode === "visual-edits") draws hit-zones; clicking selects. (4) `ATLAS_FF_REFERENCE_INPUT=true` mounts `<ReferenceDropZone />` in both PromptForm and ChatPanel; uploads go through `uploadReference` Server Action (sha256-cached under `.next/cache/atlas-references/`, served by `/api/atlas-references/[hash]/route.ts`, 5MB cap); the collected URLs ride into `startRitual.referenceImages` (Plan SPU's plumbing). (5) `ATLAS_FF_EDITABLE_PLAN=true` adds `canvas-pause`'s new `"plan-approval"` kind + `waitForPlanApproval` / `resolvePlanApproval`; renders `<PlanCheckpoints />` (inline-editable steps) and `<CritiqueDisclosure />` (collapsed `<details>` of the 5-axis findings) in the rail timeline between architect and developer rows. (6) `ATLAS_FF_ELEMENT_SLIDERS=true` lights up `<ElementInspector />` (a 64-tw-unit side panel in Visual-Edits mode): on element selection, calls `proposeElementAxes` (Haiku via the existing LLM proxy, 2-5 element-specific axes), renders one `<input type="range">` per axis; slider drag → `applyElementAxisChange` patches `/code/src/design-tokens.json` in the sandbox via E2B SDK and Tailwind rebuilds on save. CssProperty axes are runtime-only in V1.
- **Plan U: Structured triage clarifications.** When `ATLAS_FF_STRUCTURED_TRIAGE=true`, the architect's `architect.triage.needs_input` events render as an inline `<TriageClarificationForm>` (one fieldset per question; widget kind comes from the architect when declared, otherwise inferred from text — `"X or Y?"` → single-select, `"Should we ...?"` / `"Do you want ...?"` → yes-no, otherwise free text input). On submit the form serializes the answers into a hyphenated multi-line block (`"- {question} → {answer}"`) and re-runs the same `action()` / `refineAction()` ChatPanel uses for the textarea — no engine change. Flag-OFF preserves today's flat-bullet rendering byte-for-byte (assertion in `ChatPanel.test.tsx`).
- **Plan U full (slice 2, 2026-05-24).** `AmbiguityQuestionSchema` in `@atlas/role-architect` gains optional `widgetKind: "yes-no" | "single-select" | "text"` + optional `options: string[2..6]`. `TRIAGE_SYSTEM_PROMPT` instructs the LLM to declare a widgetKind for blocker questions when the answer shape is known (single-select REQUIRES options, yes-no MUST NOT have options — superRefine-enforced). `role.ts` forwards both fields into the `architect.triage.needs_input` event payload. `TriageClarificationForm.resolveQuestion` prefers the architect's declared kind over the heuristic when present (kills mis-classifications like "Pick a payment provider" being inferred as text because there's no " or " match). Engine pause/resume integration (replacing the current stop-and-restart pattern with a true pause) is a deferred third slice.
- **Plan L0: Build gate (merged 2026-05-20, PR #1, commit `b56a871`).** New `@atlas/gate-build` package runs the per-template compiler/type-checker (tsc/pyright) inside the sandbox after `sandbox.apply.completed` and BEFORE the security/a11y/visual-quality gates. `ATLAS_FF_BUILD_GATE=true` enables; default OFF. Per-template registry (`packages/gate-build/src/commands.ts`) maps each `KnownTemplate` to `{exec, parser, timeoutMs}` — all commands prefixed with `cd /code &&` so they run from the project root. On non-zero exit the gate emits `build-gate.failed {errorKind: "compile"|"type"|"timeout"|"sandbox_unreachable"|"unsupported_stack", errors: [{file, line, col, message}]}` then `build-gate.completed {passed: false, report}`; engine treats it like security/a11y and escalates via Plan L auto-fix (up to MAX_FIX_ATTEMPTS=2) with the structured errors threaded into the architect's `priorArtifact.parentBuildReport`. The architect's deep-plan prompt renders `## Build errors (compiler is authoritative — fix exactly these)` BEFORE `## Gate findings` when parentBuildReport is set. The architect's existing fix-mode bypass also recognizes parentBuildReport and synthesizes a deterministic bug-fix artifact (no LLM call) framed as a compile failure. Hybrid LLM routing committed alongside (`apps/atlas-web/lib/engine/routing-provider.ts`) — anthropic/claude-* models route to local Claude Code CLI proxy at `:3456` (free), everything else to OpenRouter. Smoke-verified end-to-end including auto-fix retry loop. **Currently flag-OFF in `.env.local`** pending D14 (diff-parser bug) + a clean end-to-end smoke (D15).
- **Canvas ModeToggle always visible (2026-05-20, commit `9580d89`).** `apps/atlas-web/components/canvas/CanvasShell.tsx` no longer gates the manifest-mode toggle behind `?canvas-modes=show`. The toggle (designing | preview) renders whenever the manifest has modes, so the user has a one-click recovery into the right mode when the EventSource-driven auto-switch misses an event. Also restructured the shell to always render its container; only the inner content swaps between EmptyCanvas (no manifest) and the active renderer. Fixes the long-running "canvas blank / I don't see options" report.
- **SSE broker now forwards `architect.triage.needs_input` + `architect.pass1.completed{passed:false}` + `build-gate.*` events.** `apps/atlas-web/lib/engine/factory.ts` `mapCheckpointToBrokerEvent`. Before this change, the architect's triage-pause emit was silently dropped — the architect rail row spun forever when the prompt mentioned "checkout flow" or other ambiguity triggers. `RitualEventType` union extended in `apps/atlas-web/lib/events/EventBroker.ts` and the corresponding type-level test.
...
| **SPU** designer 3-pass | `ATLAS_FF_DESIGNER_CRITIQUE=true` | — | DesignerRole runs draft → Haiku critique (5-axis) → Sonnet revise. +1-2 LLM calls per ritual, +quality. |
| **SPU** reference images | `ATLAS_FF_REFERENCE_IMAGES=true` | — | `StartInput.referenceImages` flows through to the architect's priorArtifact (UI uploader is `ATLAS_FF_REFERENCE_INPUT`). |
| **SPU** asset gen | `ATLAS_FF_ASSET_GEN=true` | optional `UNSPLASH_ACCESS_KEY`, optional **real** `OPENAI_API_KEY` | Engine dispatches `@atlas/role-asset-generator` after canvas pause; assetManifest folds into developer priorArtifact and render-user-turn surfaces URLs verbatim. |
| **SPU** Unsplash hero | `ATLAS_FF_HERO_UNSPLASH=true` | `UNSPLASH_ACCESS_KEY` | Asset gen calls Unsplash with the project category for hero imagery. Falls back to gradient on miss. |
| **SPU** gpt-image-1 hero | `ATLAS_FF_HERO_AI_IMAGE=true` | **Real** `OPENAI_API_KEY` (not OpenRouter — OpenRouter does NOT proxy `/v1/images/generations`) | Asset gen calls `api.openai.com/v1/images/generations` with `gpt-image-1` for hero. Cost: ~$0.04 per image. |
| **UXO** prompt morph | `ATLAS_FF_PROMPT_MORPH=true` | — | `<PromptForm />` mounts as hero on `/` for signed-in users; View Transitions morph into canvas chat input on submit. |
| **UXO** mode toolbar | `ATLAS_FF_MODE_TOOLBAR=true` | — | Three-mode (Agent / Plan / Visual-Edits) radiogroup in canvas header. Mode persisted per-project in localStorage. |
| **UXO** click to edit | `ATLAS_FF_CLICK_TO_EDIT=true` | `atlas-next-ts` template republish (for `atlas-edit-bridge.ts`) | Sandbox posts DOM tree to parent; `<IframeOverlay />` draws hit-zones when mode === "visual-edits". |
| **UXO** reference drop zone | `ATLAS_FF_REFERENCE_INPUT=true` | — | `<ReferenceDropZone />` in PromptForm + ChatPanel; uploads to `.next/cache/atlas-references/<sha>.ext`. Pair with `ATLAS_FF_REFERENCE_IMAGES` for the pipeline to actually use them. |
| **UXO** editable plan | `ATLAS_FF_EDITABLE_PLAN=true` | — | `<PlanCheckpoints />` (inline-editable steps) + collapsed `<CritiqueDisclosure />` in the rail timeline. Pair with `ATLAS_FF_DESIGNER_CRITIQUE` for findings to appear. |
| **UXO** element sliders | `ATLAS_FF_ELEMENT_SLIDERS=true` | — | `<ElementInspector />` side panel in Visual-Edits mode. Click an element → Haiku proposes 2-5 sliders → drag patches `/code/src/design-tokens.json` in the sandbox via E2B SDK. Pair with `ATLAS_FF_CLICK_TO_EDIT`. |
