...
import { useSandboxKeepalive } from "@/lib/canvas/use-sandbox-keepalive";
...
export function CanvasPreviewClient({
  ...
  // Keep the E2B sandbox warm while the canvas tab is visible — pings the
  // preview URL every 90s so E2B's ~5min idle pause never fires while the
  // user is actually looking at the canvas. Stops automatically when the
  // tab is hidden so closing it lets the sandbox expire naturally.
  useSandboxKeepalive(previewUrl);
      ...
      <div className="flex-1 min-h-0 overflow-auto flex items-start justify-center bg-slate-50 p-1">
        {previewError ? (
          ...
          <div className="flex flex-row items-stretch gap-2 max-w-full w-full h-full">
            ...
            >
              <HmrIframe key={reloadKey} src={previewUrl} title="Live preview" projectId={projectId} iframeRef={overlayIframeRef} />
              {overlayActive && (
                <IframeOverlay
              ...
              {inlineEditEnabled && imagePopoverOpen && selected && (
                <ImageReplacePopover
                  anchor={{ x: selected.rect.x, y: selected.rect.y + selected.rect.height + 8 }}
                  onClose={() => setImagePopoverOpen(false)}
                  onSubmit={async ({ url, alt }) => {
                    setImagePopoverOpen(false);
                    if (!selected.atlasId) return;
                    const atlasId = selected.atlasId;
                    // Capture the original src so we can roll back the
                    // optimistic bridge update if the source patch fails.
                    const oldUrl =
                      overlayIframeRef.current?.contentDocument?.querySelector(
                    ...
                    const result = await queue.submitPatch({
                      filePath: TARGET_FILE,
                      patch: {
                        kind: "asset-swap",
                        atlasId,
                    ...
                    if (!result.ok) {
                      // Source patch failed — roll back the live DOM so the
                      // user sees consistent state instead of a "worked then
                      // reverted on reload" mystery.
                      if (overlayIframeRef.current && oldUrl) {
                        bridgeReplaceImg(overlayIframeRef.current, {
                          atlasId,
                          newUrl: oldUrl
                        });
                      }
                      window.alert(`Replace image failed: ${result.error ?? "unknown"}`);
                    }
                  }}
                />
              )}
