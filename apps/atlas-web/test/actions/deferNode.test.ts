...
describe("deferNode action", () => {
  it("throws when ATLAS_FF_WORKFLOW feature flag is off", async () => {
    ...
    const { deferNode } = await import("@/lib/actions/deferNode");
    await expect(
      deferNode({ projectId: "p-1", workflowRunId: "wfr-1", nodeId: "n-1" })
  ...
  it("calls setNodePolicy with runMode=deferred", async () => {
    ...
    const { deferNode } = await import("@/lib/actions/deferNode");
    await deferNode({ projectId: "p-1", workflowRunId: "wfr-1", nodeId: "n-1" });
    expect(setNodePolicyFn).toHaveBeenCalledOnce();
    expect(setNodePolicyFn).toHaveBeenCalledWith("wfr-1", "n-1", { runMode: "deferred" });
  });
});
