...
- **`high`** — Reachable + operable but suboptimal:
...
- **`low`** — Best-practice nudges that DO NOT BLOCK keyboard access:
...
- [ ] Tab order matches visual order. Out-of-order → `high`.
- [ ] Modal dialogs trap focus + restore on close. Trap missing → `critical`. Restore missing → `high`.
- [ ] Arrow-key tab navigation, roving tabindex, Home/End shortcuts → `low` (nudge), unless the user explicitly asked for ARIA APG compliance.
