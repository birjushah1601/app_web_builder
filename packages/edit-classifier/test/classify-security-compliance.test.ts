...
const root = join(here, "fixtures", "edits", "security-compliance-touching");

describe("security-compliance-touching edit fixtures", () => {
  for (const dir of readdirSync(root)) {
    const fullDir = join(root, dir);
    if (!statSync(fullDir).isDirectory()) continue;
    it(`${dir} → security-compliance-touching`, () => {
      ...
      expect(result.class).toBe("security-compliance-touching");
    });
  }
});
